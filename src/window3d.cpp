...
Window3D::Window3D(QWidget* parent) : QGLWidget(parent)
{
    ...
    sg = new Scenegraph();
   xRot=180; yRot=0; zRot=180; zTra=0; nSca=1;
...
void Window3D::keyPressEvent(QKeyEvent* pe)
{
   ...
   updateGL();
        ...
        {
            if(ShiftF) params[NumberPuzzle].xPuzzlRot-=StepPuzzlRot;
            else params[NumberPuzzle].xPuzzlTra-=StepPuzzlTra;
        }
        ...
        {
            if(ShiftF) params[NumberPuzzle].yPuzzlRot-=StepPuzzlRot;
            else params[NumberPuzzle].yPuzzlTra-=StepPuzzlTra;
        }
        ...
        {
            if(ShiftF) params[NumberPuzzle].zPuzzlRot-=StepPuzzlRot;
            else params[NumberPuzzle].zPuzzlTra-=StepPuzzlTra;
        }
...
//ОСИ
   ...
   for(list<Sequence::ptr>::iterator iter=sg->sequences.begin();
       iter!=sg->sequences.end();
       iter++)
   {
        temp = *iter;
        ...
        switch(temp->getType ())
        {
            case 1: drawCam(1,0,0,0.02,0,-90,0);break;
            case 2: drawPoint3D(*rgbp,20);break;
        }
