  ...
  if (!m_size_valid)
    {
      ...
      if (m_qt_texture)
        delete m_qt_texture;
      m_qt_texture = window()->createTextureFromId(m_osg_texture_object->id(),
                                                   QSize (width(), height()));
...
void ItemView::init()
{
  ...
  m_osg_opengl_ctx->setFormat(window()->requestedFormat());
  ...
  // Z arrow
  cyl = new osg::Cylinder(osg::Vec3(0, 0, 0.5), 0.02, 1);
  s = new osg::ShapeDrawable(cyl);
  axis->addDrawable(s);
  cone = new osg::Cone(osg::Vec3(0, 0, 1), 0.04, 0.1);
  s = new osg::ShapeDrawable(cone);
  s->setColor(osg::Vec4(0, 0, 0.5, 1.0));
  axis->addDrawable(s);

  // X arrow
  cyl = new osg::Cylinder(osg::Vec3(0.5, 0, 0), 0.02, 1);
  s = new osg::ShapeDrawable(cyl);
  cyl->setRotation(osg::Quat(M_PI_2, osg::Vec3(0, 1, 0)));
  axis->addDrawable(s);
  cone = new osg::Cone(osg::Vec3(1, 0, 0), 0.04, 0.1);
  s = new osg::ShapeDrawable(cone);
  ...
  // Y arrow
  cyl = new osg::Cylinder(osg::Vec3(0, 0.5, 0), 0.02, 1);
  s = new osg::ShapeDrawable(cyl);
  cyl->setRotation(osg::Quat(M_PI_2, osg::Vec3(1, 0, 0)));
  axis->addDrawable(s);
  cone = new osg::Cone(osg::Vec3(0, 1, 0), 0.04, 0.1);
  s = new osg::ShapeDrawable(cone);
...
QSGNode * ItemView::updatePaintNode(QSGNode *, QQuickItem::UpdatePaintNodeData *)
{
  paint();
...
void ItemView::geometryChanged(const QRectF &ng, const QRectF &og)
{
